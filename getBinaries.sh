...
echoPath="/opt/ready-at-dawn-echo-arena"
mkdir -p $echoPath



function updateEcho {
    if ! [[ -f ./files/rsyncHosts ]]
    then
        echo "The rsyncHosts-file is missing. The script will be stopped now. It needs to be located at $PWD/files"
        echo -e '\033[0m' # No Color
        exit
    fi
    readarray -t rsyncHosts < ./files/rsyncHosts
    serverCounter=0
    ...
    #Start the download
    $rsyncCommand${rsyncHosts[$fastestServer]}/. /opt/ready-at-dawn-echo-arena
}
...
function checkForUpdates {
    ...
    then
        echo -e '\033[0;31m' #write in red
        wall "The rsyncHosts-file is missing. The script will be stopped now. It needs to be located at $PWD/files"
        echo -e '\033[0m' # No Color
        exit
    fi
    echo -e '\033[0;31m' #write in red
    ...
    do
        ...
        then
            echo "No Echo-Updateserver reachable. Exit"
            exit
        fi
        ...
        if ! [[ $rsyncCheck ]]
        then
            echo "no Update found. All files are correct."
            exit
        fi
